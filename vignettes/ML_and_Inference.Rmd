---
title: "Introduction to the R Package TDApplied"
...
- distinguishing between the topology of human brain function of healthy control subjects vs. subjects with a neurological disorder [@TDA_ADHD;@TDA_Alzheimers;@persistent_hom_brain_networks],
...
- improving model predictions of various chemical properties of molecules by including topological features [@TDA_chemistry], etc. 

For a broad introduction to the mathematical background and main tools of topological data analysis, see [@TDA_intro].

Distance and kernel functions are used to quantify differences and similarities respectively between objects in complicated spaces, and are the foundation of many machine learning algorithms which are used in traditional data science [@murphyML]. Several papers have used distance and kernel computations tailored to persistence diagrams [@distance_calc;@persistence_fisher] in order to perform machine learning or inference tasks with groups of persistence diagrams, but to date no publicly available software in either R or python provides the functionality for these types of analyses.

Two main R packages exist for computing and comparing persistence diagrams -- TDA [@R-TDA] and TDAstats [@R-TDAstats]. The paper [@PH_benchmarking] compared the runtimes of the homology calculations in the R packages TDA and TDAstats, and concluded that certain persistent homology calculations are faster with TDAstats, however calculating persistence diagram remains a major bottleneck for topological data analyses. Importantly, neither package supports machine learning with groups of persistence diagrams because both lack a kernel function (between R and python only the python package scikit-TDA [@scikittda2019] supports kernel calculations). TDAstats has an inference method; however, it is based on a non-standard distance function (making inferences drawn from this function unsubstantiated). TDA has a standard distance function but no inference function (and also stores persistence diagrams in a non-standard data structure, which requires additional preprocessing steps for further analysis).  Several other packages exist in R and python for vectorizing persistence diagrams and analyzing the resulting vectors (TDAvec [@TDAvec], TDAkit [@TDAkit] and giotto-tda [@tauzin2020giottotda]), but such processes remove some of the information in the diagrams. 
...
3. quantify differences and similarities between pairs of persistence diagrams with high speed, 
...
In order to achieve these goals TDApplied includes the following functions:

1. `PyH` connects with python creating a fast persistent (co)homology engine compared to alternatives.
2. `diagram_to_df` converts the output of persistent homology calculations from TDA and TDAstats to a data frame.
3. `diagram_distance` and `diagram_kernel` allow for fast distance and kernel calculations respectively.
4. `plot_diagram` and `bootstrap_persistence_thresholds` can be used to plot and threshold persistence diagrams (for "significant" features).
5. `diagram_mds`, `diagram_kpca`, `diagram_kkmeans` and `diagram_ksvm` perform machine learning with persistence diagrams (multidimensional scaling, kernel principal components analysis, kernel k-means clustering and support vector machines respectively) and `permutation_test` and `independence_test` perform inference on groups of diagrams (a permutation test for group differences and a two-sample independence test respectively).

In accomplishing the five goals TDApplied is a comprehensive and optimized package which permits researchers and data scientists to analyze multiple data sets using machine learning and inference via topological data analysis. This vignette documents the background, theory and limitations of TDApplied functions on simulated data.
...
```{r,echo = F,fig.height = 3,fig.width = 7,fig.align = 'center',eval = requireNamespace("TDA")}
...
In this section we will describe the various computational tools implemented in the package TDApplied to analyze persistence diagrams. While we will discuss certain "fast" functions in the package, benchmarking and details of computational speedups are documented in another vignette. To start we must load the TDApplied package:
...
Examples will either analyze pre-computed data frames, TDA/TDAstats calculated persistence diagrams, or simple diagrams which are random deviations of three persistence diagrams called D1, D2 and D3 (each with points in dimension 0 only):
...
## Fast Calculation of Persistence Diagrams

The long calculation time required to calculate persistence diagrams in a major limiting factor in applied topological data analyses. In order to reduce this computational bottleneck TDApplied provides a persistent cohomology engine via python, which produces the exact same output as persistent homology just much faster [@PChom;@PHom_dualities]. The ripser module in python is a wrapper for the same persistent cohomology engine as the R package TDAstats, which has provided until now the fastest persistence diagram calculation function in R [@PH_benchmarking]. However, it was observed in simulations that the python implementation seemed faster, even when called via the reticulate package. Therefore, the `PyH` function can be used as a wrapper of the python ripser module for fast Vietoris-Rips filtration persistent cohomology.
...
The ripser module is imported outside of the `PyH` function using the `import_ripser` function, because this operation can be slow and if many persistence diagrams are to be computed then repeating this operation would be highly prohibitive. Additionally, the `import_ripser` function checks the three prerequisites for `PyH` listed earlier in this section, to avoid repeating checks. In the TDApplied package testing it was verified that the `PyH` output matches the TDAstats `calculate_homology` output for the same input data and parameters, except for one small difference: in the python ripser calculation there is always one connected component (i.e. dimension 0 topological feature) with infinite death value, whereas this feature is omitted in the TDAstats calculation. Whether or not to include this feature in the outputted diagram can be decided by the user via the `ignore_infinite_cluster` parameter being set to `TRUE` (the default value) or `FALSE` respectively.
...
## Convert Diagrams to Data Frames

The most typical data structure used in R for data science is a data frame. The output of the `PyH` function is a data frame (unless representatives are calculated in which case the output is a list containing a data frame and other information),but the persistence diagrams calculated from the R packages TDA and TDAstats are not stored in data frames, making subsequent analyses of these diagrams challenging. In order to solve this problem the TDApplied function `diagram_to_df` can convert TDA/TDAstats persistence diagrams into data frames (this example will not run without the TDA and TDAstats packages installed):

```{r,echo = T,eval = requireNamespace("TDAstats") & requireNamespace("TDA")}
...
## Quantifying Differences and Similarities Between Pairs Persistence Diagrams

As the persistence diagram is a descriptor of the underlying shape structure of a dataset, it can be useful to quantify the difference and similarity between pairs of persistence diagrams. Difference can be quantified with distance functions and similarity with kernel functions, and these types of functions are the foundation of all applied analysis methods of persistence diagrams. 
...
Before analyzing data it is often best practice to first visualize and clean the data. TDApplied has one function for plotting persistence diagrams and another for removing points from persistence diagrams which are not "significant".

### Plotting Diagrams

Persistence diagrams can contain any number of points representing different types of topological features from different homological dimensions. However we can easily view this information in a two-dimensional plot of the birth and death values of the topological features, where each homological dimension has a unique point shape and color, using the `plot_diagram` function:

```{r,echo = T,fig.height = 5,fig.width = 5,fig.align = 'center'}
D_plot <- data.frame(dimension = c(0,1,1,2),birth = c(0,0.1,0.1,0.2),death = c(2,0.2,0.3,0.25))
plot_diagram(D_plot,title = "Example plot")
```

### Finding Real Topological Features via Bootstrapping

Noise in datasets can drastically affect the results of inference and machine learning procedures, therefore it is desirable to clean data before applying such procedures. Noise in persistence diagrams are low persistence topological features, i.e. features who's birth and death values are very similar (such points would be near the diagonal line when plotted). The question of determining which points in a persistence diagrams are "significant" and which are "noise" has been addressed via a bootstrapping approach in [@bootstrap].
...
Hypothesis testing [@CasellaBerger] is a mathematical framework for making inferences about statistics, and is a common first approach for analyzing data. TDApplied has implemented two hypothesis testing procedures.
...
There are three main limitations of TDApplied which should be discussed for its own future improvements. The first limitation is that the fast approximation to the Fisher information metric (and therefore to the persistence Fisher kernel) cannot be run in parallel. Unfortunately it is not possible (to our knowledge) to export a c++ function for parallel usage within a package - therefore other packages may implement similar distance/gram matrix and Turner loss functions with parallelization of c++ diagram distances to achieve significant performance gains.  The second limitation is in the function `diagram_ksvm` - the only acceptable input is a dataset where the single training feature is a persistence diagram (one diagram for each training example). This may be too inflexible for some applications, where the training features may include several persistence diagrams, or a mix of persistence diagrams, numeric and categorical features. A future update to TDApplied should provide the functionality for allowing any number of training features, of various types, to the `diagram_ksvm` function via a weighted sum of kernels. The third limitation of TDApplied is the long runtime of its `diagram_distance` function compared to python's persim `wasserstein` function. Even with a potential runtime increase caused by the reticulate package, the persim `wasserstein` function was significantly faster than the TDApplied `diagram_distance` function. As such, a future version of TDApplied could attempt to speed up its distance calculations in order to be competitive with python implementations (for example with the Rcpp package). While TDApplied has been created with flexibility and scalability at its core, there will always be room for adding more functionality and speedups.

# Conclusion

The TDApplied package aims to bridge topological data analysis with researchers and data practitioners in the R community. Current topological data analysis packages in R and python do not provide the ability to carry out standard types of data analysis, being statistics and machine learning, with persistence diagrams - greatly limiting research and industry interest in topological data analysis. TDApplied was built with performance in mind, with its own fast persistent (co)homology wrapper,  quick native-R implementations of distance calculations between persistence diagrams, C++ approximate kernel calculations and parallelization at several points. Topological data analysis is an exciting and powerful new field of data analysis, and with TDApplied anyone can access its power for meaningful and creative analyses of data.

# Appendix: Proof of Correctness for Distance Calculation
