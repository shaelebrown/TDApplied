...
In accomplishing these goals TDApplied is a comprehensive and optimized package which permits researchers and data scientists to analyze multiple data sets using machine learning and inference via topological data analysis.
...
2. normalizing the resulting matrices to have maximum value 1, 
3. computing thresholded persistence diagrams of dimension 1 from those dissimilarity matrices (using the bootstrap method), and 
4. using machine learning and inference functions from TDApplied to analyze the resulting collection of diagrams.
...
As we can see the distributions were very different between these three subjects. Next we visualized the nine tasks:
...
Again we found that the tasks had visually different distributions, with the most pronounced differences when $\sigma = 0.001$. 

Next we analyzed basic properties of the persistence diagrams - the number of loops and the mean persistence of loops, both across subjects and tasks. The overall distribution of the number of loops across all diagrams was:
...
Behavioral measures may correlate with the mental state topologies, and therefore may be explaining some of the variance within the plots (like how 0-dimensional topological features of spatial FC were found to correlate with behavior [@topological_time_and_space]), and this could be an interesting future analysis. Also cluster 1 seemed to be more 2 dimensional in both embeddings whereas cluster 2 seemed to be more one dimensional, and determining what factors could explain these dimensions would also be an interesting future analysis.

## Conclusions of HCP Analysis

Our inferences analysis results found that major and non-trivial task-related 1-dimensional topological features may be largely conserved across people, but with high variability, and that the topology of tasks were largely different but also highly non-independent. Our machine learning analysis revealed that three main types of mental state topologies occurred across all subjects and tasks, and that these clusters were highly separable and likely either 1 or 2 dimensional (and none of the dimensions were explained by simple external factors). Taken together, these results point towards a small number of neural computational strategies which are largely consistent between people but also highly interrelated across domains, which is consistent with neuroscience literature [@PON,WHAT].
