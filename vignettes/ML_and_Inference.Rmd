...
The function `bootstrap_persistence_thresholds` can be used to determine these persistence thresholds. Here is an example for the circ dataset, and the results can be plotted with the `plot_diagram` function:

```{r,fig.height = 4,fig.width = 8,fig.align = 'center'}
# calculate the bootstrapped persistence thresholds using 2 cores
# and 30 iterations
thresh <- bootstrap_persistence_thresholds(X = circ,FUN = "ripsDiag",
...
# plot first graph
plot_rips_graph(gs,1L)

# plot second graph
plot_rips_graph(gs,2L)
```
