...
While TDAML is designed to use as input the output of TDA persistent homology calculations, **TDAML is also able to use as input the output of the TDAstats function `calculate_homology`, provided that its output is first converted to a data frame using the `as.data.frame` function**. The kernel machine learning methods implemented in TDAML are wrappers of the flexible R package for kernel calculations kernlab [@R-kernlab], with some additional processing steps specific to persistence diagrams. In the subsequent sections we will describe these applications in more detail.
...
# Benchmarking the TDAML `diagram_distance` Function against Python
...
# plot table
plot(summary_table$n_row[summary_table$package=="persim"], 
...
lines(summary_table$n_row[summary_table$package=="TDAML"],
      summary_table$mean[summary_table$package=="TDAML"], 
      col=2, type="b")
legend(x = 200,y = 2000,legend = c("TDAML","persim"),
       col = c("red","black"),lty = c(1,1),cex = 0.8)
arrows(summary_table$n_row, 
       summary_table$mean-1.96*summary_table$sd/sqrt(10), 
       summary_table$n_row, summary_table$mean+1.96*summary_table$sd/sqrt(10),
