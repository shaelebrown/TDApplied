...
par(mfrow = c(1,3))
plot_diagram(D1,title = "D1",xlim = c(0,4),ylim = c(0,4),legend = F)
plot_diagram(D2,title = "D2",xlim = c(0,4),ylim = c(0,4),legend = F)
plot_diagram(D3,title = "D3",xlim = c(0,4),ylim = c(0,4),legend = F)
par(mfrow = c(1,1))
```
...
# Fast Persistent Homology Calculations Using Reticulate

While the focus of TDApplied is carrying out applied topological data analysis with persistence diagrams computed with the R packages TDA and TDAstats, TDApplied also provides its own persistent homology engine via python. The ripser module in python is a wrapper for the same persistent homology engine as the R package TDAstats, which is currently the state-of-the-art of persistent homology calculation speed in R. However, it was observed by the authors that the python implementation seemed faster, even when called via the reticulate package. Therefore, the `PyH` function is a wrapper of the python ripser module for Vietoris-Rips filtration persistent homology.
...
## Future Work: Persistent Cohomology with Python

Persistent cohomology is another tool from topological data analysis which computes persistence diagrams from point clouds, but even faster than persistent homology and localizing topological features concretely in data space (@PChom, @PHom_dualities). This makes persistent cohomology an attractive alternative to persistent homology for applied topological data analysis, so why is it not included in TDApplied? The dionysus python module has a persistent cohomology function for point clouds, however due to the complex data structure in which the results are stored (essentially a list of "diagram point" python objects) the fast conversion of the output of dionysus calculations remains an open problem (currently the fastest that we could find is `lapply` and `do.call` function calls). We include here example code for calculating the persistent cohomology of a circle data set, with the hopes that in the future someone will find a way to either adapt the conversion to a data frame, or to write a persistent cohomology function from scratch in R. Here is our current example:

```{r,echo = T,eval = F}
# create circle dataset
circ = TDA::circleUnif(n = 50,r = 1)

# download and import the dionysus python module
reticulate::py_install("dionysus")
dionysus = reticulate::import("dionysus")

# calculate persistent cohomology using dionysus up to dimension 1 and radius 2
f = dionysus$fill_rips(reticulate::np_array(data = circ,dtype = 'float'),k = 1L,r = 2)
p = dionysus$cohomology_persistence(f)
dgms = dionysus$init_diagrams(p,f)

# convert output to data frame by creating one dataframe for each dimension and then
# combining with rbind
dgm0 <- do.call(rbind,lapply(X = 0:(length(dgms[[1]])-1),FUN = function(X){
  
  return(data.frame(dimension = 0,birth = dgms[[1]][[X]]$birth,death = dgms[[1]][[X]]$death))
  
}))
dgm1 <- do.call(rbind,lapply(X = 0:(length(dgms[[2]])-1),FUN = function(X){
  
  return(data.frame(dimension = 1,birth = dgms[[2]][[X]]$birth,death = dgms[[2]][[X]]$death))
  
}))
diag <- do.call(rbind,list(dgm0,dgm1))
```

Note that this code was not found to be nearly as fast as the `PyH` function on some circle data sets, pointing towards the need of a proper persistent cohomology function in R. Moreover, if homology dimension 2 is also calculated then the disparity in run time is even greater.

# Limitations of TDApplied

There are three main limitations of TDApplied which should be discussed for its own future improvements. The first limitation is in the function `diagram_ksvm` - the only acceptable input is a dataset where the single training feature is a persistence diagram (one diagram for each training example). This may be too inflexible for some applications, where the training features may include several persistence diagrams, or a mix of persistence diagrams, numeric and categorical features. A future update to TDApplied should provide the functionality for allowing any number of training features, of various types, to the `diagram_ksvm` function via a weighted sum of kernels. The second limitation of TDApplied is the long runtime of its `diagram_distance` function compared to python's persim `wasserstein` function. Even with a potential runtime increase caused by the reticulate package, the persim `wasserstein` function was significantly faster than the TDApplied `diagram_distance` function. As such, a future version of TDApplied could attempt to speed up its distance calculations in order to be competitive with python implementations. Finally, while TDApplied has implemented a fast persistent homology calculation via reticulate, the faster persistent cohomology currently has no implementation, and this should be the focus of a future update. While TDApplied has been created with flexibility and scalability at its core, there will always be room for adding more functionality and speedups.
