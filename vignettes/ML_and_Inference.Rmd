...
```{r}
circ_result <- universal_null(circ,thresh = enclosing_radius(circ))
...
Certainly not! All real world datasets will contain some measure of noise and therefore will likely have more than one feature in a given homological dimension. Let's add some noise to our circ dataset and rerun the procedure:

```{r}
# create circle with noise dataset and plot
circ_with_noise <- circ
circ_with_noise$x <- circ_with_noise$x + stats::rnorm(n = 50,sd = 0.1)
circ_with_noise$y <- circ_with_noise$y + stats::rnorm(n = 50,sd = 0.1)
...
Here's an example of performing inference on the noisy circ dataset, with and without infinite cycle inference at a lower threshold value (and a slightly higher `alpha`):
