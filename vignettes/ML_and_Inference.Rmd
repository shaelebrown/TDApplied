...
In this section we will describe the various computational tools implemented in the package **TDApplied** to analyze persistence diagrams. To start we must load the **TDApplied** package:

```{r setup}
library("TDApplied")
``` 

Our examples will take as input persistence diagrams computed in one of three ways: (1) using the fixed diagrams D1, D2 and D3 from the diagram below, (2) using D1, D2 and D3 with added Gaussian noise to their points, or (3) **TDA**/**TDAstats** calculated persistence diagrams from simple shapes like points sampled from a unit circle.
...
In this vignette we have implemented a helper function `generate_TDApplied_vignette_data` which will add Gaussian noise with a small variance to the birth and death values of the points in D1, D2 and D3, making "noisy copies" of the three diagrams.
...
The **ripser** module is imported outside of the `PyH` function using the `import_ripser` function, because this command checks the `PyH` requirements and can be slow -- if many persistence diagrams are to be computed then repeating such checks would be highly prohibitive. Additionally, the `import_ripser` function checks the three prerequisites for `PyH` listed earlier in this section, to avoid repeating checks. In the **TDApplied** package testing it was verified that the `PyH` output matches the **TDAstats** `calculate_homology` output for the same input data and parameters, except for one small difference: in the Python **ripser** calculation there is always one connected component (i.e. dimension 0 topological feature) with infinite death value, whereas this feature is omitted in the **TDAstats** calculation. Whether or not to include this feature in the outputted diagram can be decided by the user via the `ignore_infinite_cluster` parameter being set to `TRUE` (the default value) or `FALSE`.
...
## Converting Diagrams to Data Frames
