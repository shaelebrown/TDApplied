...
ADD HCP PH paper!! persistent homology time and space...

To demonstrate the utility of TDApplied we analyzed neurological data from the Human Connectome Project [@HCP]. The Human Connectome Project contains a huge amount of high quality neuroimaging data collected from about 1200 subjects, which allows researchers to make powerful inferences about the (function of the) human brain. Of the data collected, functional MRI was used to record brain activity over time while participants were doing a number of tasks, including two resting-state scans (i.e. absence of task), an emotion task, a gambling task, a language task, a working memory task, a relational task, a social task and a motor task (see [@HCP] for the details of each task), and for each task there were two scans (resulting in 18 scans per subject). 

The resting state scans were longer than the other task scans, which normally necessitates the use of analysis methods which abstract out temporal information (like forming graphs of inter-regional time-course correlations and comparing graphs across tasks [@twos_company]). However, by using topology we can interrogate the geometry of the "mental states" (activity patterns) without abstracting over time, therefore not losing temporal information. This type of analysis has previously been carried out with another tool from topological data analysis called mapper [@mapper], producing graph structures of clusters of mental states during task and rest [@BrainOrganizationMapper]. However, mapper has multiple parameters to choose, must cluster over time points, and cannot find higher-dimensional structure which likely exists in fMRI data [@twos_company]. Importantly, loops in such an analysis may represent task-related (periodic) processing. Moreover, it was found that persistent (co)homology has the ability to detect topological signal in fMRI data [@topological_feasibility,@cohomology_feasability], and that functional connectivity (correlation) graphs contained robust topological features when preprocessed with ICA-fix which correlated with various measures of WHAT CITE

For these reasons we decided to analyze 100 HCP subject's fMRI data with TDApplied. To this end we 

1. calculated time-point by time-point correlation dissimilarity matrices of each fMRI dataset for each subject, 
2. normalized the resulting matrices to have maximum value 1, 
3. computed thresholded persistence diagrams of dimension 1 from those dissimilarity matrices (using the bootstrap method), and 
4. used machine learning and inference functions from TDApplied to analyze the resulting collection of diagrams.
...
```{r,echo = F}
data.frame(t = c(0.0249876874399648,0.0329454617744531,0.042378103713154,0.0598132151928147,0.0998022585335929,0.165384794916381,0.0249876874399648,0.0329454617744531,0.042378103713154,0.0598132151928147,0.0998022585335929,0.165384794916381,0.0249876874399648,0.0329454617744531,0.042378103713154,0.0598132151928147,0.0998022585335929,0.165384794916381,0.0000979929586891795,0.000171993496642182,0.000312640568618485,0.000440414334000389,0.000859076303540747,0.00156929386612317,0.0000979929586891795,0.000171993496642182,0.000312640568618485,0.000440414334000389,0.000859076303540747,0.00156929386612317,0.0000979929586891795,0.000171993496642182,0.000312640568618485,0.000440414334000389,0.000859076303540747,0.00156929386612317,0.000000445913341102581,0.000000903949194437481,0.00000228157654069726,0.00000391982119010704,0.00000793999813378195,0.0000151232398106584,0.000000445913341102581,0.000000903949194437481,0.00000228157654069726,0.00000391982119010704,0.00000793999813378195,0.0000151232398106584,0.000000445913341102581,0.000000903949194437481,0.00000228157654069726,0.00000391982119010704,0.00000793999813378195,0.0000151232398106584),sigma = rep(rep(c(0.001,0.01,0.1),each = 6),3))
```

## Comparing subjects and tasks

Some functions of healthy human brains are conserved across people, and other functions are not CITE. But are task-induced mental state topologies conserved across subjects or tasks? To answer this question we analyzed distributional properties of the persistence diagrams between subjects and between tasks, and compared the diagrams between subjects and between tasks using the permutation and independence tests.

Firstly we visualized the persistence diagrams of subjects and tasks by combining all the diagrams for that subjects/task and plotting the distribution incurred by adding a Gaussian point mass centered at each point (like in the Fisher information metric). Using the $\sigma$ values 0.001, 0.01 and 0.1. Plots with $\sigma = 0.1$ were least informative because it was comparable to the scale of the data. Below we show the diagrams of three subjects and all tasks with $\sigma = 0.001$:

```{r,echo = F,eval = T,fig.height = 3,fig.width = 9,fig.align = 'center'}
par(mfrow = c(1,3))
visualize_diagram <- function(diag,sigma,plot_title){
...
visualize_diagram(s105620,sigma = 0.001,plot_title = "Subject 105620")
visualize_diagram(s117122,sigma = 0.001,plot_title = "Subject 117122")
visualize_diagram(s118023,sigma = 0.001,plot_title = "Subject 118023")
```

```{r,echo = F,eval = T,fig.height = 9,fig.width = 9,fig.align = 'center'}
par(mfrow = c(3,3))
visualize_diagram(rest1,sigma = 0.001,plot_title = "Resting state 1")
visualize_diagram(rest2,sigma = 0.001,plot_title = "Resting state 2")
visualize_diagram(emotion,sigma = 0.001,plot_title = "Emotion")
visualize_diagram(gambling,sigma = 0.001,plot_title = "Gambling")
visualize_diagram(language,sigma = 0.001,plot_title = "Language")
visualize_diagram(motor,sigma = 0.001,plot_title = "Motor")
visualize_diagram(relational,sigma = 0.001,plot_title = "Relational")
visualize_diagram(social,sigma = 0.001,plot_title = "Social")
visualize_diagram(wm,sigma = 0.001,plot_title = "Working memory")
```

We analyzed basic properties of the persistence diagrams - the number of loops, the persistence of loops and the birth and death values of the loops, both across subjects and tasks. The overall distribution of the number of loops across all diagrams was:
...
barplot(num_loops,xlab = "Number of loops")
```

The bootstrap thresholding was evidently very conservative, with about 73\% of the diagrams containing no loops. This is not to say that those diagrams were topologically trivial - on the contrary they may have contained non-trivial higher-dimensional topological structure, or it is possible that task-correlated loops may exist at smaller (i.e. less persistent) scales. To assess the validity of either explanation, which would be an interesting question to explore in the future, would require significantly more computational resources than what was available at the time of writing. On the other hand, the fact that many diagrams did not have any loops provides evidence that topological structure in the fMRI data was not driven by physiological sources, such as heart-rate and breathing, which is known to drive signal in fMRI data CITE.

We then compared the distribution of the number of loops across subjects and tasks using paired t-tests. About 19\% of the subject-subject pairs had statistically different numbers of loops, and about 22\% of the task-task pairs had statistically significant numbers of loops (all at the $\alpha = 0.05$ significance level using a two-sided alternative hypothesis). Interestingly, most of the differences between tasks were found between resting state scans and task scans, with less loops in the resting state scans, suggesting that tasks may induce large-scale and periodic cycles of mental states which are used to accomplish the task.

Next we then performed the same analysis for the mean persistence of loops in the diagrams. About 12\% of the subject-subject pairs had differences in mean persistence values. We found 14\% statistically different persistence between tasks, mainly being lower persistence in resting state scans compared to task-based scans.

In order to better understand subject-subject and task-task differences we used the permutation test to compare subjects (matched by task) and tasks (matched by subjects) with both the 2-wasserstein and bottleneck distances, judging significance at the $\alpha = 0.05$ level. For the subjects, both bottleneck and wasserstein distances found about 20\% of the subject-subject pairs to have different topologies, with about 96\% of the differences conserved between the two metrics. For the tasks, in the bottleneck tests there were 4 pairs of different tasks: both resting state scans with the emotion task, the first resting scan with the social task and the emotional task with the relational task. For the wasserstein tests there were 8 pairs of different tasks: the first resting state scan with the emotion, gambling, motor and relational tasks, the second resting state scan with the emotion, motor and working memory tasks, and the relational task with the working memory task. Therefore, the resting state scans were more different from task scans than the task scans were with each other. 

Next we analyzed whether any of the subject-subject or task-task pairs were independent (again at the $\alpha = 0.05$ significance level). Only about 18% of the subject-subject pairs were significantly non-independent (and an interesting question for the future would be whether these pairs were blood-related). On the other hand, over 86% of the task-task pairs were found to be significantly non-independent in at least half of the kernel parameter values. Therefore, task-induced topology may be strongly related between tasks. In order to compare the results of the two inference procedures, we used a two-sided t-test to compare the percentage of kernel values found with significant non-independence between subject-subject pairs which were significantly different in the permutation test vs. not. The results were very similar for both distance metrics, with significantly lower percentage of non-independence in non-different subject-subject pairs.

## Machine learning

Our initial analyses revealed some of the underlying topological features/drivers of neural function in the HCP dataset, however without being able to visualize all of the persistence diagrams simultaneously our story is not complete and compelling. In this section we will visualize the diagrams in 2D using MDS computed from both the bottleneck and wasserstein metrics. We will then "correlate" these low dimensional embedding coordinates with external features, including sex, 5-year age bin, and cluster label, by visual inspection. We start by calculating cluster labels, as we describe below.

Using all of the kernel parameters deduced earlier, we performed kernel kmeans clustering on all the persistence diagrams over a range of values of k - 2,3,4,...,18,25,50,100. These values were chosen because there were 18 scans per subject and 100 subjects. The within-cluster sum of squared distances were calculated for each value of k and for each kernel parameter combination, and the value of k which minimized the withinss (without resulting in empty clusters) was selected as being optimal for that kernel parameter. The results were:
...
From the graph it seemed like the optimal number of clusters was likely either 3 or 4. We performed clustering with both values of k for all kernel parameters, and calculated for each pair of diagrams for what percentage of kernel parameters that pair was in the same cluster. For k equals 3 we found that about 54\% of the pairs were in the same cluster at least 90\% of the time, and that 31\% of the pairs were in the same cluster less than 5\% of the time. Similarly for four clusters, 53\% of the pairs were in the same cluster at least 90\% of the time, and 34\% percent of the pairs were in the same cluster less than 5\% of the time. These results indicate that the clusterings for k equalling 3 and 4 were highly consistent across all kernel parameters. We then used the kernlab function `kkmeans` to cluster the two results, and only for k = 3 was the clustering able to be resolved (potentially due to the matrix not being a real kernel matrix). We therefore visualized the group of diagrams in each of the three found clusters:
...
visualize_diagram(cluster1,sigma = 0.001,plot_title = "Cluster 1")
visualize_diagram(cluster2,sigma = 0.001,plot_title = "Cluster 2")
visualize_diagram(cluster3,sigma = 0.001,plot_title = "Cluster 3")
```

The three clusters are quite distinct in terms of what topological information they likely represent. For cluster one this is mainly two loops but sometimes a small or very large loop (mean persistence of about 0.0018), for cluster two this is one loop of variable size (mean persistence of about 0.0011), and cluster 3 is one main large loop and sometimes on small loop (this group only had 16 loops in it, with mean persistence about 0.0005).
...
Cluster 1 had much higher percentages of resting state scans compared to the other clusters. However, it also had lower percentages of emotion, gambling and social tasks than the other clusters. Clusters 2 and 3 were moreso separated by percentages of resting state 2 (more in cluster 3), gambling (more in cluster 2), social (more in cluster 2) and motor (more in cluster 3).

We then calculated MDS embeddings of all the diagrams using both the wasserstein and bottleneck distances. No variable like task, subject ID, subject gender or subject age clearly separated the embedded points, however the cluster labels did:
...
No clear separation could be seen visually using task, subject ID, subject age or subject gender as the point color in the two plots, suggesting that other variables, like behavioral measures, may be impacting the topology. This could serve as an interesting future analysis. Also cluster 1 seemed to be more 2 dimensional in both embeddings whereas cluster 2 seemed to be more one dimensional, and determining what factors could explain these dimensions would also be an interesting future analysis.

## Conclusions of HCP Analysis

Taking into account our distributional analysis, permutation test analysis and independence test analysis, our results suggest that major 1-dimensional topological features may be largely conserved across people in tasks and subjects, however there were some major differences in the 1-dimensional topologies of the mental states across tasks and subjects which were tied together. Moreover, topology of tasks were largely different but highly non-independent. Taken together our results point towards a shared "manifold" of mental states which is mainly modulated by task, but also modulated by subject in a structured way. Our machine learning analysis revealed that about three main types of "loopy" spaces occurred across all the subjects and tasks, separated by certain tasks but containing examples of all tasks. MDS embeddings of all the diagrams were able to show that clusters 1 and 2 were likely 2 and 1 dimensional respectively, but that no basic variable was able to explain either of its dimensions, either requiring interactions or more nuanced variables like behavioral measures. Our analysis points towards a rich task and subject dependent topology of mental states during task which are non-trivial and largely low-dimensional. 
