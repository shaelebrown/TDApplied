...
Now this plot doesn't quite look like how we would expect - it's not a proper loop! In order to fix this we need to subset the edges we plot by only plotting edges which have distance less than the death value of the loop. This value was 1.738894, so we will pick a threshold of 1.7 (other representatives for the same loop can be found by picking a different threshold, as long as the threshold is at least as big as the birth value of the loop):
...
Since TDA's `bootstrapDiagram` function uses the same bootstrap procedure for persistence thresholds as TDApplied's `bootstrap_persistence_thresholds`, it will be left for a future version to benchmark these two functions.
...
This completes the proof that the solution to the LSAP problem on $M$ has the same cost as the bottleneck distance value between Diag1 and Diag2.
