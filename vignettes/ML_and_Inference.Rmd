...
It is worthwhile to note that there are two other R packages for carrying out topological data analysis - TDAvec [@TDAvec] (methods for vectorizing persistence diagrams) and TDAkit [@TDAkit] (clustering and dimension reduction methods for functional summaries of persistence diagrams, called persistence landscapes/silhouettes). Topological summaries like persistence landscapes have their own suite of statistical methods [@statistics_on_landscapes;@landscapes_toolbox] due to their simpler underlying space compared to that of persistence diagrams [@Robinson_Turner]. There are also several python packages dedicated to topological data analysis calculations, including scikit-TDA [@scikittda2019] (which computes persistence diagrams, and distances/kernels between them, using several python libraries), and giotto-tda [@tauzin2020giottotda] (which computes persistence diagrams and uses them to analyze time series data).
...
There is a generalization of the 2-wasserstein distance for any $p \geq 1$, the $p$-wasserstein distance, which can also be computed using the `diagram_distance` function by varying the parameter `p`. To see the computational details and proof of correctness for wasserstein and bottleneck distance calculations, see Appendix A.
...
# Speeding up Analyses with Precomputed Gram/Distance Matrices

Despite the various speedups implemented in TDApplied (like parallelization, a fast native distance function, etc.), it is an unavoidable issue that applied topological data analysis methods are generally slow. The runtime limiting factor is the calculation of distance/Gram matrices (especially Gram matrices..), because distance and kernel calculations can individually be slow, and these matrices are calculated in every inference and machine learning function of TDApplied. If we are performing multiple analyses of the same persistence diagrams with the same distance/kernel parameters this slow-down would compound - we would be needlessly recomputing distance/Gram matrices for the different functions. To avoid this issue TDApplied inference and machine learning functions have the capability to accept precomputed distance/Gram matrices, and we would expect runtimes of $k$ analyses using the same distance/Gram matrix to have a runtime of approximately $1/k$ times the runtime without using precomputed matrices, since we would be computing 1 out of the $k$ (redundant) matrices. We timed the analyses in this section on the same computer as in the previous section.

As an example suppose we have a set of nine persistence diagrams and we want to analyze them with MDS ,kernel k-means and kernel PCA:
...
mds <- diagram_mds(diagrams = g,k = 2,dim = 0,sigma = 1.5,distance = "fisher",num_workers = 2)
                              
# calculate kmeans clusters with 3 centers
clust <- diagram_kkmeans(diagrams = g,centers = 3,dim = 0,t = 2,sigma = 1.5,num_workers = 2)

# calculate kpca embedding
pca <- diagram_kpca(diagrams = g,dim = 0,t = 2,sigma = 1.5,features = 2,num_workers = 2)
```

The time taken to run the MDS, k-means and PCA lines was about Xs. Noting that the distance/Gram matrices had shared parameters (i.e. the distance matrix was calculated with the Fisher information metric and the values of t and sigma were all shared), we then repeated the analysis by pre-computing one distance (and Gram) matrix and using these in all three analyses:

```{r,eval = F,echo = T}
D <- distance_matrix(diagrams = g,dim = 0,sigma = 1.5,distance = "fisher",num_workers = 2)
K <- exp(-2*D)
class(K) <- "kernelMatrix"

# calculate MDS embedding
mds <- diagram_mds(D = D,k = 2,num_workers = 2)

# calculate kmeans clusters with 3 centers
clust <- diagram_kkmeans(diagrams = g,K = K,centers = 3,dim = 0,t = 2,sigma = 1.5,num_workers = 2)

# calculate kpca embedding
pca <- diagram_kpca(diagrams = g,K = K,dim = 0,t = 2,sigma = 1.5,features = 2,num_workers = 2)
```

The new runtime (including calculating the distance and Gram matrices) was Ys, much faster than the original. We then repeated the analyses using 300 persistence diagrams, i.e. with
...
The timing scaled proportionally - without using precomputed matrices the time taken was Xs and with precomputed matrices the time taken was Ys. We recommend using precomputed matrices whenever performing multiple similar analyses of the same persistence diagrams, and we did exactly this in the following section where we analyzed neurological data with TDApplied.
...
# Appendix A: Proof of Correctness for Distance Calculation
...
# Appendix B: Converting Correlations to Distances
