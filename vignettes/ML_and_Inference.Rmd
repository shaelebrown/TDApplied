...
Now that we can visualize persistence diagrams we will describe three tools which can be used for their interpretation -- filtering out noisy topological features with bootstrapping, representative (co)cycles and *Rips graphs*.
...
# the p-values are both 1/31 (about 0.032) which is the minimum possible
# when num_samples = 30 
thresh$pvals
...
# compute two Rips graphs
gs <- rips_graphs(X = circ,eps = c(eps_1,eps_2))
```

Next we can plot both Rips graphs:

```{r,eval = requireNamespace("igraph"),fig.height = 5,fig.width = 5,fig.align = 'center'}
# plot first graph
plot_rips_graph(gs,eps_1)

# plot second graph
plot_rips_graph(gs,eps_2)
```

The first graph shows that at a scale before the loop is born, the dominant loop does not exist in the data (and that the space is more fragmented), while the second graph was able to retrieve the dominant loop. Visualizing multiple Rips graphs of a dataset, choosing particular scales of interest from the persistence diagram of the dataset, can be an effective tool for interpreting topological features.

The input parameters of `plot_rips_graph` can help customize the graph visualizations. For example, if we want to investigate the loop we can also customize the plot to highlight the loop representative, and to only plot the component of the graph which contains the loop vertices:
...
plot_rips_graph(gs,eps_2,cols = colors,component_of = stimuli_in_loop[[1]])
```
...
# plot only component containing the loop stimuli with vertex colors
graph_info <- plot_rips_graph(gs,eps_2,cols = colors,
                component_of = stimuli_in_loop[[1]],
                vertex_labels = F,return_layout = T)

# get layout
layout <- graph_info$layout
...
vertex_inds <- match(stimuli_in_loop,graph_info$vertices)
