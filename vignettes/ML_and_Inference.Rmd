...
STILL TO DO:
1. Fix image formatting in this vignette
2. Change all the tests to be the same as in this vignette
3. Change parallel code to be able to run with only 2 cores for CRAN (set as parameter with checks everywhere)
4. Check package build on R devel and multiple platforms
...
In this section we will describe the various computational tools implemented in the package TDAML to analyze persistence diagrams, both explaining the mathematics and providing functional examples. All examples will analyze simple diagrams which are random deviations of three persistence diagrams called D1, D2 and D3 (each with points in dimension 0 only):
...
When desired, random Gaussian noise with a small variance will be added to the birth and death values of the points in these three diagrams (being careful make sure the points always have appropriate birth and death values), resulting in a "noisy copy". The code used in this vignette to obtain noisy copies is:

```{r,echo = T}
noisy_copy <- function(diag){
  t <- diag
  t$dimension <- as.numeric(as.character(t$dimension))
  t$birth <- t$birth + stats::rnorm(n = nrow(diag),mean = 0,sd = 0.05)
  t[which(t$birth < 0),2] <- 0
  t$death <- t$death + stats::rnorm(n = nrow(diag),mean = 0,sd = 0.05)
  t[which(t$birth > t$death),2] <- t[which(t$birth > t$death),3]
  return(t)
}
```

To run our examples we must start by loading the TDAML package:
...
g <- list(noisy_copy(D1),noisy_copy(D1),noisy_copy(D1),
          noisy_copy(D2),noisy_copy(D2),noisy_copy(D2),
          noisy_copy(D3),noisy_copy(D3),noisy_copy(D3))

# calculate their 2D MDS embedding in dimension 1 with the bottleneck distance
mds <- diagram_MDS(diagrams = g,dim = 0,p = Inf,k = 2)
...
```{r,echo = T}
# permutation test between all three diagrams with 20 iterations
perm_test <- permutation_test(list(noisy_copy(D1),
                                   noisy_copy(D1),
                                   noisy_copy(D1),
                                   noisy_copy(D1),
                                   noisy_copy(D1)),
...
clust <- diagram_kkmeans(diagrams = g,centers = 3,dim = 0,t = 2,sigma = 2)
...
pca <- diagram_kpca(diagrams = g,dim = 0,t = 2,sigma = 2,features = 2)
...
g <- lapply(X = 1:30,FUN = function(X){
  ...
  return(noisy_copy(get(paste0("D",(X %% 3) + 1))))
  
})

# create response vector
y <- as.factor(rep(c("D1","D2","D3"),10))

# fit model with cross validation
model_svm <- diagram_ksvm(diagrams = g,cv = 2,dim = c(0),y = y,sigma = c(1,0.1),t = c(1,2))
```
...
g_new <- lapply(X = 1:9,FUN = function(X){
...
D1_copies <- lapply(X = 1:10,FUN = function(X){noisy_copy(D1)})
D2_copies <- lapply(X = 1:10,FUN = function(X){noisy_copy(D2)})

# do independence test with sigma = t = 1
indep_test <- independence_test(D1_copies,D2_copies,dims = c(0))
indep_test$p_value
```
