...
2. Then we repeatedly sample, with replacement, the original data and compute new persistence diagrams $\{D_1,\dots,D_n\}$.
3. We calculate the bottleneck distance of each new diagram with the original, either in each dimension separately (obtaining one threshold for each dimension) or for all points altogether.
4. Finally we compute the $1-\alpha$ percentile of these values (in each dimension or altogether as was calculated).

This/these thresholds is/are the maximum bottleneck distance from any point which the geometry of the persistence diagram changes significantly, forming a "confidence interval" around each point in $D$ (where the bottleneck distance to that point is at most the threshold, forming a square around the point). Therefore, "real" points will be those whose confidence intervals do not touch the diagonal line, where birth and death is the same, so the persistence threshold is twice the bottleneck distance threshold calculated above.
...
(4) Determining persistence thresholds with TDApplied's `bootstrap_persistence_thresholds` and TDA's `bootstrapDiagram`.

While it would be ideal to benchmark kernel calculations as well, there is no current R implementation and the python implementation is for a different kernel then what is used in TDApplied. Since no comparison to other functions is reasonable, we leave it for future work to further optimize the speed of the `diagram_kernel` function.
...
As we can see the run time of `PyH` was significantly less than that of `calculate_homology`. We then used a linear model to analyze how the two functions scale compared to each other for all three shapes:
...
A linear model was used to verify that `diagram_distance` scaled better than `wasserstein` (with about a 95x speed up for 1000 data points), suggesting that distance calculations with TDApplied are faster and more scalable: 
...
The runtime of the persim `wasserstein` function was significantly faster than TDApplied's `diagram_distance` function. However, a linear model of the run time ratio of TDApplied vs. persim against the number of points in the shape finds evidence that the two functions scale similarly (via a constant run time ratio of mean 15):
