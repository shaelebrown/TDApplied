...
It is with these goals that the package TDAML was created to build upon the TDA package, however persistence diagrams computed with the TDAstats package can also be used by TDAML with only minimal preprocessing (see the section "Package TDAML" for details).
...
While TDAML is designed to use as input the output of TDA persistent homology calculations, **TDAML is also able to use as input the output of the TDAstats function `calculate_homology`, provided that its output is first converted to a data frame using the `as.data.frame` function** (for example, `as.data.frame(calculate_homology(X))`). The kernel machine learning methods implemented in TDAML are wrappers of the flexible R package for kernel calculations kernlab [@R-kernlab], with some additional processing steps specific to persistence diagrams. In the subsequent sections we will describe these applications in more detail.
...
arrows(summary_table$n_row[summary_table$package == "TDAML"], 
       summary_table$mean[summary_table$package == "TDAML"]
       -1.96*summary_table$sd[summary_table$package == "TDAML"]/sqrt(10),
       summary_table$n_row[summary_table$package == "TDAML"], 
       summary_table$mean[summary_table$package == "TDAML"]
       +1.96*summary_table$sd[summary_table$package == "TDAML"]/sqrt(10), 
...
arrows(summary_table$n_row[summary_table$package == "TDAML"], summary_table$mean[summary_table$package == "TDAML"]-1.96*summary_table$sd[summary_table$package == "TDAML"]/sqrt(10), summary_table$n_row[summary_table$package == "TDAML"], summary_table$mean[summary_table$package == "TDAML"]+1.96*summary_table$sd[summary_table$package == "TDAML"]/sqrt(10), length=0.05, angle=90, code=3,col = "red")
...
A linear model can be used to verify that the runtime ratio of the TDA vs. TDAML grew at each subsequent number of tested data points in the shapes (with about a 100x speed up for 1000 data points), suggesting that distance calculations with TDAML are faster and more scalable: 
...
# plot table
plot(summary_table$n_row[summary_table$package=="TDAML"],
...
lines(summary_table$n_row[summary_table$package=="persim"],
      ...
      col="black", type="b")
legend(x = 200,y = 20,legend = c("TDAML","persim"),
       col = c("red","black"),lty = c(1,1),cex = 0.8)
arrows(summary_table$n_row[summary_table$package == "TDAML"], 
       summary_table$mean[summary_table$package == "TDAML"]
       -1.96*summary_table$sd[summary_table$package == "TDAML"]/sqrt(10),
...
The runtime of the persim `wasserstein` function was significantly faster than TDAML's `diagram_distance` function. However, a linear model of the runtime ratio of TDAML vs. persim against the number of points in the shape finds evidence that the two functions scale similarly (via a constant runtime ratio of mean 15):
...
Nevertheless, the raw speed increase in python could be the basis for a very fast python counterpart to the TDAML package in the future.

# Limitations of TDAML

There are two main limitations of TDAML which should be discussed for its own future improvements. The first limitation is in the function `diagram_ksvm` - the only acceptable input is a dataset where the single training feature is a persistence diagram (one diagram for each training example). This may be too inflexible for some applications, where the training features may include several persistence diagrams, or a mix of persistence diagrams, numeric and categorical features. A future update to TDAML should provide the functionality for allowing any number of training features, of various types, to the `diagram_ksvm` function via a weighted sum of kernels. The second limitation of TDAML is the long runtime of its `diagram_distance` function compared to python's persim `wasserstein` function. Even with a potential runtime increase caused by the reticulate package, the persim `wasserstein` function was significantly faster than the TDAML `diagram_distance` function. As such, a future version of TDAML could use the wasserstein (and bottleneck) distance calculations from the persim module as its distance engine, however this would introduce a couple of complications. Firstly, a dependency on reticulate, and therefore on python, would be introduced which is not ideal. Secondly, since the Fisher information metric is not implemented in the persim module, calculating this metric would still require its own R code. While TDAML has been created with flexibility and scalability at its core, there will always be room for adding more functionality and speedups.
