...
A large cluster of surface nodes responding to $\theta$ was found in the left hemisphere, which appeared to belong to the brain region Pol1 in the insular cortex. Interestingly, Pol1 was not found to show significant differences in activity between the faces and shapes conditions [@HCP_multimodal_parcellation], despite the implication of the insular cortex in emotional processing [@insula], suggesting that spatial loops of fMRI data may contain complementary information to typical task-based analyses of fMRI data. 
...
```{r,echo = F,out.width="80%", out.height="80%",fig.show="hold",fig.align = 'center'}
knitr::include_graphics(c("theta_activity.png"))
# knitr::include_graphics(c("r_activity.png"))
```

A similar plot would have been possible for different values of $r$, along a single straight-line axis, however with so few nodes we found that the plot was unclear. A t-test then found a significant difference in mean $r$ values between shape and face blocks:
