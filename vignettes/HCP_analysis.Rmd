...
Again we found that the tasks had visually different distributions, although the two resting state scans are fairly similar, as we would expect, with one main cluster of loops of birth and death just under 0.2.
...
Using TDApplied we were able to analyze fMRI data spatially without summarizing (i.e. removing) temporal information. We identified meaningful topological features of fMRI datasets which varied across tasks and subjects in structured patterns, and found that fewer neurological dimensions may be needed to summarize fMRI data than previously thought. Applied topological data analysis unlocks new and exciting ways for analyzing (neuroimaging) data which was not previously possible.
