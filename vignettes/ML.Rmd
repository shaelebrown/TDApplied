...
While the TDA package, based on the C++ package Dionysus, computes the bottleneck distance between matched points, another popular alternative is to compute Euclidean (or Minkowski) distances between matched points, like in the Robinson and Turner paper. The Euclidean case can be acheived by setting $p=2$ for the "Turner" distance, which should result in a distance of $\sqrt{0.1^2+0.5^2 + 0.5^2} \approx 0.7141428$:

```{r}
diagram_distance(D1 = D1,D2 = D2,distance = "fisher",dim = 0,sigma = 1)
```

Again, it is easy to switch to a Minkowski metric by changing the $p$ parameter. If we set $p=3$ the distance should be $(0.1^3+0.5^3+0.5^3)^{1/3} \approx 0.6307994$:
