...
`TDApplied` has already been featured in a [conference workshop](https://github.com/WoComtoQC/wocomtoqc.github.io/blob/main/abstract.md), utilized in a journal publication [@Yash] and downloaded over 3700 times. Therefore, we propose `TDApplied` as a user-friendly, efficient and flexible R package for the analysis of multiple datasets using machine learning and inference via topological data analysis.
...
*Code quality:* Code has been tested using the `testthat` package [@R-testthat], with 91.53\% coverage of R code when not skipping tests involving Python code (or 88.45\% coverage when skipping the Python tests).

*Documentation:* `TDApplied` contains five main vignettes: 

1. "Introduction to TDApplied" provides example function usage on simulated data as well as mathematical background and intuition, 
