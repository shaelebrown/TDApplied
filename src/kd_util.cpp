...
//----------------------------------------------------------------------
//	annBoxDistanceFlops - utility routine which computes distance from point to
//		box (Note: most distances to boxes are computed using incremental
//		distance updates, not this function.)
//  Added by Vlad 5-1-08 to allow computation of flops if user desires it
//    at runtime (while still keeping the fast version that does not compute flops).
//----------------------------------------------------------------------

ANNdist annBoxDistanceFlops(			// compute distance from point to box
    const ANNpoint		q,				// the point
    const ANNpoint		lo,				// low point of box
    const ANNpoint		hi,				// high point of box
    int					dim)			// dimension of space
{
  ...
  ANN_FLOP_ALWAYS(4*dim)						// increment floating op count
