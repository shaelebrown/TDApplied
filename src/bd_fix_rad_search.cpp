...
void ANNbd_shrink::ann_FR_searchFlops(ANNdist box_dist)
{
  // check dist calc term cond.
  if (ANNmaxPtsVisited != 0 && ANNptsVisited > ANNmaxPtsVisited) return;
  ...
  if (inner_dist <= box_dist) {				// if inner box is closer
    child[ANN_IN]->ann_FR_searchFlops(inner_dist);// search inner child first
    child[ANN_OUT]->ann_FR_searchFlops(box_dist);// ...then outer child
  }
  else {										// if outer box is closer
    child[ANN_OUT]->ann_FR_searchFlops(box_dist);// search outer child first
    child[ANN_IN]->ann_FR_searchFlops(inner_dist);// ...then outer child
  }
  ANN_FLOP_ALWAYS(3*n_bnds)							// increment floating ops
    ANN_SHR(1)									// one more shrinking node
}