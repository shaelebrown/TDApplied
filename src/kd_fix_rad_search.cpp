...
// these are added by Vlad to report nn's found during range search
//   with out spending any time sorting them
ANNidxArray fr_search_indexes;
ANNdistArray fr_search_dists;
int fr_search_unordered;
int fr_search_k; // this his here so we know the length of the arrays above
    ...
    double				eps)			// the error bound
{
  ...
  ANN_FLOP(2)							// increment floating op count
    ...
    fr_search_unordered = 0; // added by Vlad so that the leaf nodes know where
  // to put pts (either in priority queue or in array; this case pq)
...
// Added by Vlad to update flops even when ANN_PERF is not defined
//----------------------------------------------------------------------
//	annkFRSearch - fixed radius search for k nearest neighbors
//----------------------------------------------------------------------

int ANNkd_tree::annkFRSearchFlops(
    ANNpoint			q,				// the query point
    ANNdist				sqRad,			// squared radius search bound
    int					k,				// number of near neighbors to return
    ANNidxArray			nn_idx,			// nearest neighbor indices (returned)
    ...
    int * flops )			// the number of floating ops performed by this query
{
  ...
  if( flops != NULL )
    *flops = ann_Nfloat_ops;
  
  ANNkdFRMaxErr = ANN_POW(1.0 + eps);
  ANN_FLOP_ALWAYS(2)							// increment floating op count
    
    fr_search_unordered = 0; // added by Vlad so that the leaf nodes know where
  // to put pts (either in priority queue or in array; this case pq)
  ANNkdFRPointMK = new ANNmin_k(k);	// create set for closest k points
  // search starting at the root
  root->ann_FR_searchFlops(annBoxDistanceFlops(q, bnd_box_lo, bnd_box_hi, dim));
...
// Added by Vlad 5-1-08 to update flops even when ANN_PERF is not defined
//----------------------------------------------------------------------
//	kd_split::ann_FR_search - search a splitting node
//		Note: This routine is similar in structure to the standard kNN
//		search.  It visits the subtree that is closer to the query point
...
void ANNkd_split::ann_FR_searchFlops(ANNdist box_dist)
{
  // check dist calc term condition
  if (ANNmaxPtsVisited != 0 && ANNkdFRPtsVisited > ANNmaxPtsVisited) return;
  ...
  if (cut_diff < 0) {					// left of cutting plane
    child[ANN_LO]->ann_FR_searchFlops(box_dist);// visit closer child first
    ...
    // visit further child if in range
    if (box_dist * ANNkdFRMaxErr <= ANNkdFRSqRad)
      child[ANN_HI]->ann_FR_searchFlops(box_dist);
    
  }
  else {								// right of cutting plane
    child[ANN_HI]->ann_FR_searchFlops(box_dist);// visit closer child first
    ...
    // visit further child if close enough
    if (box_dist * ANNkdFRMaxErr <= ANNkdFRSqRad)
      child[ANN_LO]->ann_FR_searchFlops(box_dist);
    
  }
  ANN_FLOP_ALWAYS(13)						// increment floating ops
    ANN_SPL(1)							// one more splitting node visited
}
...
void ANNkd_leaf::ann_FR_search(ANNdist box_dist)
{
  ...
  for (int i = 0; i < n_pts; i++) {	// check points in bucket
    ...
    if (d >= ANNkdFRDim &&					// among the k best?
      ...
      // add it to the list
      if( fr_search_unordered == 0 )
      {
        // add new neighbor and its distance to priority queue
        ANNkdFRPointMK->insert(dist, bkt[i]);
      }
      else
      {
        // add new neighbor index and its distance to the unordered arrays
        if( fr_search_indexes != NULL && ANNkdFRPtsInRange < fr_search_k )
          fr_search_indexes[ANNkdFRPtsInRange] = bkt[i];
        if( fr_search_dists != NULL && ANNkdFRPtsInRange < fr_search_k )
          fr_search_dists[ANNkdFRPtsInRange] = dist;
      }
...
// Added by Vlad to allow update of flops even when ANN_PERF is not
//   defined, at the same time keeping the fast version of the fcn above
//----------------------------------------------------------------------
//	kd_leaf::ann_FR_search - search points in a leaf node
//		Note: The unreadability of this code is the result of
//		some fine tuning to replace indexing by pointer operations.
...
void ANNkd_leaf::ann_FR_searchFlops(ANNdist box_dist)
{
  ANNdist dist;				// distance to data point
  ANNcoord* pp;				// data coordinate pointer
  ANNcoord* qq;				// query coordinate pointer
  ...
  for (int i = 0; i < n_pts; i++) {	// check points in bucket
    ...
    for(d = 0; d < ANNkdFRDim; d++) {
      ANN_COORD(1)				// one more coordinate hit
      ANN_FLOP_ALWAYS(5)					// increment floating ops
    ...
    if (d >= ANNkdFRDim &&					// among the k best?
      ...
      {
        // add new neighbor and its distance to priority queue
        ANNkdFRPointMK->insertFlops(dist, bkt[i]);
      }
      else
      {
        // add new neighbor index and its distance to the unordered arrays
...
//
//
// Functions, added by Vlad to allow for unordered fixed radius search where all neighbors within
//  radius are reported.  This reduces the cost since we do not need to keep neighbors sorted.
//
//  6/10/08
//
//
...
int ANNkd_tree::annkFRSearchUnordered(
    ANNpoint			q,				// the query point
    ANNdist				sqRad,			// squared radius search bound
    int					k,				// number of near neighbors to return
    ANNidxArray			nn_idx,			// nearest neighbor indices (returned)
    ...
    double				eps)			// the error bound
{
  ...
  ANN_FLOP(2)							// increment floating op count
    ...
    // set the flag that we are doing unordered search
    fr_search_unordered = 1;
  fr_search_indexes = nn_idx;
  fr_search_dists = dd;
  fr_search_k = k;
...
int ANNkd_tree::annkFRSearchUnorderedFlops(
    ANNpoint			q,				// the query point
    ANNdist				sqRad,			// squared radius search bound
    int					k,				// number of near neighbors to return
    ANNidxArray			nn_idx,			// nearest neighbor indices (returned)
    ...
    int * flops )			// the number of floating ops performed by this query
{
  ...
  // search starting at the root
  root->ann_FR_searchFlops(annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim));
