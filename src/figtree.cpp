...
//------------------------------------------------------------------------------
// change some functions (Rprintf, new, delete) to use Matlab functions
//   instead if we are compiling library for use in matlab mex file.
//------------------------------------------------------------------------------
#ifdef FIGTREE_USE_MATLAB_MEX
#include "mex.h"

// use mexRprintf instead of Rprintf for messages
// #undef Rprintf
...
if( (VAR) <= 0.0 )                                                   \
{                                                                    \
  Rprintf( #FCN ": Input '" #VAR "' must be a positive number.\n");   \
  return -1;                                                         \
}
...
if( (VAR) == NULL )                                      \
{                                                        \
  Rprintf( #FCN ": Input pointer '" #VAR "' is NULL.\n"); \
  return -1;                                             \
}
                            ...
                            double * clusterCenter, double * maxSourceDists2, int * pMaxTotals, double * C )
{
  ...
  //Rprintf( "source p histogram: ");
  //for( int i = 0; i < pMax; i++ )
  //  Rprintf( " %i", pHistogram[i] );
  //Rprintf( "\n" );
  //delete [] pHistogram;
...
// [[Rcpp::export]]
std::vector<double> figtree( std::vector<double> X, double h, 
                  std::vector<double> Q, std::vector<double> Y, double epsilon, std::vector<double> G)
{
  
  int d, N, M, W;
  int evalMethod, ifgtParamMethod, ifgtTruncMethod, verbose;
  evalMethod = 4;
  ifgtParamMethod = 1;
  ifgtTruncMethod = 2;
  verbose = 0;
      ...
      evalMethod == FIGTREE_EVAL_IFGT_TREE )
  {
    if(verbose && evalMethod == FIGTREE_EVAL_IFGT)
      Rprintf("figtreeEvalMethod() chose the IFGT method.\n");
    if(verbose && evalMethod == FIGTREE_EVAL_IFGT_TREE)
      Rprintf("figtreeEvalMethod() chose the IFGT+tree method.\n");
    ...
    if( !alreadyHaveClustering )
    {
      ...
      if( ret < 0 )
        Rprintf("figtree: figtreeKCenterClustering() failed.\n");
    }
    ...
    if( ret >= 0 && !alreadyHaveClustering )
    {
      ...
      if( ret < 0 )
        Rprintf("figtreeChooseTruncatoinNumber() failed.\n");
      else
      {
        if( verbose && errorBound > epsilon )
          Rprintf("figtreeChooseTruncationNumber(): could not find p within limit that satisfies error bound!\n" );
      }
    } 
    ...
    if( ret >= 0 )
    {
      ...
      if( data.pMax == 1 && (ifgtTruncMethod != FIGTREE_TRUNC_MAX) )
      {
        ...
        //verbose && Rprintf("figtree(): max truncation is 1, so adaptive truncations are unnecessary.\n  Switching to FIGTREE_TRUNC_MAX...\n");
      }
      ...
      if( W > 1 && (ifgtTruncMethod == FIGTREE_TRUNC_CLUSTER ) )
      {
        ...
        //verbose && Rprintf( "figtree(): FIGTREE_TRUNC_CLUSTER is not yet implemented\n  to handle W > 1.  Switching to FIGTREE_TRUNC_POINT...\n");
      }
                             ...
                             double r, double epsilon, double * g )
{
  ...
  Rprintf("This code was not compiled with support for ANN.  Please recompile\n");
  Rprintf("with 'FIGTREE_NO_ANN' not defined to enable ANN support.\n");
  return -1;
#else
  // check input arguments
...
#ifdef FIGTREE_NO_ANN
  Rprintf("This code was not compiled with support for ANN.  Please recompile\n");
  Rprintf("with compiler flag 'FIGTREE_NO_ANN' not set to enable ANN support.\n");
  return -1;
#else
  // check input arguments 
                                                   ...
                                                   double * avgNbrClustersNoAnn, double * avgFindCentersFlops )
{
  ...
  {
    int targetBase = (Rcpp::sample(M,1)[0] - 1)*d;        
    for(int k = 0; k < K; k++)
    {
      int centerBase = k*d;
      double targetCenterDistanceSquare = 0.0;
                                   ...
                                   int * bestMethod, double * flops, void * data_struct )
{
  ...
  std::vector<int> numbvec = as<std::vector<int>>(Rcpp::sample(N,N));
  for( int i = 0; i < N; i++ )
    shuffled_indexes[i] = numbvec[i];
  ...
  if( kLimit > 0 )
  {
    ...
    if( ret >= 0 )
    {
      ...
      if( ret >= 0 )
      {
        ...
        //verbose && Rprintf( "Eval IFGT(h= %3.2e, pMax= %i, K= %i, r= %3.2e, rx= %3.2e, epsilon= %3.2e, bound = %3.2e)\n", 
      ...
      {
        Rprintf("figtree: figtreeChooseTruncationNumber() failed.\n");
      }
