...
public:
  ...
  // added by Vlad 5-1-08 to allow user to update flops by calling this
  // function even when ANN_PERF is not defined
  inline void insertFlops(					// insert item (inlined for speed)
      PQKkey kv,						// key value
      PQKinfo inf)					// item info
  {
    int i;
    ...
    mk[i].info = inf;
    if (n < k) n++;				// increment number of items
    ANN_FLOP_ALWAYS(k-i+1)				// increment floating ops
  }
};
