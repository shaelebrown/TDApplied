...
test_that("diagram_kpca detects incorrect parameters correctly",{
  ...
  expect_error(diagram_kpca(diagrams = list(D,D,D[,1:2])),"three")
  expect_error(diagram_kpca(diagrams = list(D,D,D),t = -1),"t")
  expect_error(diagram_kpca(diagrams = list(D,D,D),sigma = 0),"sigma")
  expect_error(diagram_kpca(diagrams = list(D,D,D),dim = NULL),"dim")
...
test_that("diagram_kpca is computing correctly",{
  ...
  eig <- eigen(K)
  kpca <- diagram_kpca(diagrams = list(D1,D2,D3),features = 2)
...
test_that("predict_diagram_kpca detects incorrect parameters correctly",{
  ...
  D3 <- data.frame(dimension = 0,birth = c(2,5),death = c(3.1,6))
  kpca <- diagram_kpca(diagrams = list(D1,D2,D3),features = 2)
  expect_error(predict_diagram_kpca(new_diagrams = list(),kpca),"1")
  expect_error(predict_diagram_kpca(new_diagrams = NA,kpca),"NA")
  expect_error(predict_diagram_kpca(new_diagrams = list(diagrams[[1]],1),kpca),"diagram")
  expect_error(predict_diagram_kpca(new_diagrams = list(D1,D2,D3),embedding = 2),"kpca")
...
test_that("predict_diagram_kpca is computing correctly",{
  ...
  D3 <- data.frame(dimension = 0,birth = c(2,5),death = c(3.1,6))
  kpca <- diagram_kpca(diagrams = list(D1,D2,D3),features = 2)
  expect_equal(predict_diagram_kpca(new_diagrams = list(D1,D2,D3),embedding = kpca),kpca$pca@rotated)
